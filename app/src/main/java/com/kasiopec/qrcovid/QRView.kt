...
@Singleton
class QRView @Inject constructor(private val context: Context, private val prefsManager: PrefsManager) {
    ...
    fun recreateQrCodeFromBitmap(bitmap: Bitmap): ImageBitmap? {
        ...
        //Return ImageBitmap, return null if decoded nothing or exception
        return try{
            val qrCodeResult = MultiFormatReader().decode(binaryBitmap) ?: return null
            //Saving QR Code result to SharedPrefs
            prefsManager.setCovidPassCode(qrCodeResult.text)

            val newQrBitmap = barcodeEncoder.encodeBitmap(
                qrCodeResult.text,
                BarcodeFormat.QR_CODE,
                600.dpToPx(context),
                600.dpToPx(context)
            )
            Log.d("QrView", "I am called")
